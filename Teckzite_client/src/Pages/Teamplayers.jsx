...
const rotateAnimation = keyframes`
  0% {
    transform: perspective(50000px) rotateY(0deg);
  }
  100% {
    transform: perspective(50000px) rotateY(360deg);
  }
`;
...
const Players = () => {
  ...
  const fetchPlayers = async () => {
    try {
      ...
      console.error(error);
      toast.error('Something went wrong while fetching players');
    } finally {
      setLoading(false);
    }
  };
  ...
  const rotationAngle = filteredPlayers.length > 0 ? 360 / filteredPlayers.length : 0;
      ...
      <Content>
        <div className="min-h-screen text-white">
          <div>
            ...
            <div className="flex flex-row justify-center w-full gap-5   mt-[250px] md:mt-[100px]">
            <AnimatedBox>
              {filteredPlayers.length > 0 ? (
                filteredPlayers.map((player, index) => (
                    // <span style={{ '--i': index, ...spanStyle }} key={index}>
                    <span
                    style={{
                      '--i': index,
                      ...
                      transformOrigin: 'center',
                      transformStyle: 'preserve-3d',
                      transform: 'rotateY(calc(var(--i) * var(--rotation-angle))) translateZ(400px)',
                      WebkitBoxReflect: 'below 1px linear-gradient(transparent,transparent,#0004)',
                    }}
                    key={index}
                  >
                    <div className="flex flex-row justify-center" style={imgStyle}>
                        <div
                          className={`bg-[#161929]  border rounded-lg overflow-hidden shadow-md hover:bg-[#161929] cursor-pointer transition-colors duration-300 ease-in-out ${player.isSold
                              ? 'shadow-[0_0_15px_rgba(6,182,212,0.3)] border-cyan-500'
                ...
                <p className="text-center text-red-400">No players found.</p>
              )}
               </AnimatedBox>
            </div>
          </div>
