...
import { FaFlag, FaUser, FaDollarSign, FaUsers } from 'react-icons/fa';
  ...
  const fetchData = async () => {
    try {
      const [playersResponse, teamResponse] = await Promise.all([
        fetch(`http://localhost:8000/api/getteamplayers/${id}`),
        fetch(`http://localhost:8000/api/getTeaminfo/${id}`)
      ]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1">
          {filteredPlayers.length > 0 ? (
            filteredPlayers.map((player, index) => (
              ...
              >
                <div
                  className={` from-cyan-900 via-black to-gray-900 border rounded-lg overflow-hidden shadow-md hover:bg-[#161929] cursor-pointer transition-colors duration-300 ease-in-out ${player.isSold
